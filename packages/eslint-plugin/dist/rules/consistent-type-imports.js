        ...
        function* fixToTypeImportDeclaration(fixer, report, sourceImports) {
            ...
            if (typeNamedSpecifiers.length) {
                if (sourceImports.typeOnlyNamedImport) {
                    ...
                    const sourceEndsWithSemicolon = sourceCode.getTokenAfter(node.source)?.value === ';';
                    ...
                    if (fixStyle === 'inline-type-imports') {
                        yield fixer.insertTextBefore(node, `import {${typeNamedSpecifiers
                            ...
                            .join(', ')}} from ${sourceCode.getText(node.source)}${sourceEndsWithSemicolon ? ';' : ''}\n`);
                    }
                    else {
                        yield fixer.insertTextBefore(node, `import type {${fixesNamedSpecifiers.typeNamedSpecifiersText}} from ${sourceCode.getText(node.source)}${sourceEndsWithSemicolon ? ';' : ''}\n`);
                    }
                }
            }
            ...
            if (namespaceSpecifier &&
                ...
                // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ insert
                yield fixer.insertTextBefore(node, `import type ${sourceCode.getText(namespaceSpecifier)} from ${sourceCode.getText(node.source)}${sourceEndsWithSemicolon ? ';' : ''}\n`);
            }
            if (defaultSpecifier &&
                report.typeSpecifiers.includes(defaultSpecifier)) {
                if (report.typeSpecifiers.length === node.specifiers.length) {
                    ...
                    const defaultText = sourceCode.text
                        ...
                        .trim();
                    const sourceEndsWithSemicolon = sourceCode.getTokenAfter(node.source)?.value === ';';
                    yield fixer.insertTextBefore(node, `import type ${defaultText} from ${sourceCode.getText(node.source)}${sourceEndsWithSemicolon ? ';' : ''}\n`);
            ...
            if (isDefaultImport) {
                ...
                if (openingBraceToken) {
                    ...
                    if (node.specifiers.length > 1) {
                        const sourceEndsWithSemicolon = sourceCode.getTokenAfter(node.source)?.value === ';';
                        yield fixer.insertTextAfter(node, `\nimport type${specifiersText} from ${sourceCode.getText(node.source)}${sourceEndsWithSemicolon ? ';' : ''}\n`);
                    }
                }
            }
